...
regionclassification <- read_tsv("WorldRegions (1).csv",skip = 1)
...
covid_response_new <- covid_response %>% 
  select(`CountryName`,CountryCode,Date,`C2_Workplace closing`,`C5_Close public transport`,`C4_Restrictions on gatherings`,`C6_Stay at home requirements` ,ConfirmedCases,GovernmentResponseIndex,EconomicSupportIndex)

covid_response_tidy <- covid_response_new[rowSums(is.na(covid_response_tidy)) == 0, ]
```
...
Analyzing Area 
We decided to look at how Area affects covid response 
```{r}
area
...
```
Merging Area and covid response
```{r}
...
glimpse(area_covid_response_Date2)
```
Now we will use interquartile range in order to find the outliers
```{r}
boxplot.stats(area_covid_response_Date1$GovernmentResponseIndex)$out
```
Looking at the outcome of the code above you can tell that there are a total of 7 datapoints that are considered outliers in 2021-04-28. Now we will look at where all the other data points in this variable fall into 
```{r}
ggplot(area_covid_response_Date1) +
  aes(x = "", y = GovernmentResponseIndex) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()
```
Majority of the data are between 50 and 70 which makes it conserning that 7 countries have such a low Government response to covid. Now lets so which countries are the outliers
```{r}
area_covid_response_Date1 %>%
  filter(GovernmentResponseIndex < 21)
```
As you can see most of these are countries with a fairly low ranking, but the three outliers here seem to have the lowest government response yet they are ranked higher then the countries with a high response by the government. 
...
```
Now looking at the data 4 months later there is less outliers but the response has lowered only by a small amount. There seems to be only 2 outliers, which reveals that more countries have stepped up between the two months. 
```{r}
area_covid_response_Date2 %>%
  filter(GovernmentResponseIndex < 21)
```
Out of the seven only two counties have the lowest government response 

Now we will look at the correlation between the governmnet response and Area
